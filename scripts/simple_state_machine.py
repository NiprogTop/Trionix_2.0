...
class Emerge(smach.State):
    def __init__(self, c):
        smach.State.__init__(self, outcomes=['abortedd'])
        self.cntr = c
...
if __name__ == '__main__':
    ...
    c = Controller()
    sm = smach.StateMachine(outcomes=['WAIT'])
    with sm:
        smach.StateMachine.add('WAIT', Wait(c),
                               transitions={'start': 'SUBMERGE'})

        smach.StateMachine.add('SUBMERGE', Submerge(c, 0.0),
    ...
    sm.execute()
    rospy.loginfo("State Machine #### END ####  Node")
